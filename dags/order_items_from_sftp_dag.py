...
endpointName="order_items"
target_table_name="order_item"
DB_SCHEMA = "stage"
...
target_table = Table(
    ...
    Column("pickup_limit_date", String(26)),
...
SFTP_FILE_PATH = "/upload/order_item_dataset.csv"
...
def load_data_to_db(ti):
    ...
    # Get a list of column names that are of type String with a length
    string_columns = [(col.name, col.type.length) for col in target_table.columns if isinstance(col.type, String)]
    ...
    for record in api_data:
        ...
        # Handle string length validation and truncation
        for col_name, max_len in string_columns:
            value = record.get(col_name)
            if value is not None and len(value) > max_len:
                print(f"WARNING: Truncating string for column '{col_name}'. Original length: {len(value)}, max length: {max_len}. Value: '{value}'")
                record[col_name] = value[:max_len]
